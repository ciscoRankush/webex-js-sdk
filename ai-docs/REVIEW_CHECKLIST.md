...
|---|---|---|---|
| C1 | Spec-currency + WHAT/WHY | Spec/docs changed in the same change as code; every requirement (incl. ADDED) states WHAT and WHY | Blocking |
| C2 | Contract correctness | Provides/Requires delta is real and complete; no undocumented breaking change to a public plugin/aggregate surface | Blocking |
...
| C5 | Error handling + input validation | Untrusted input validated at plugin boundaries; failure/edge paths handled, not swallowed | Important |
| C6 | Security baseline | No hardcoded secrets; tokens/key material/plaintext never logged (per `SECURITY.md`) | Blocking |
...
|---|---|---|---|---|
| K1 | Regression guard | Modifying a weakly covered module, or any MODIFIED/REMOVED requirement | A characterization baseline exists; invariants the change claims NOT to alter still hold | Blocking |
| K2 | Grounding | Weakly covered module (all `Partial` here until host coverage-review) | Claims cite real code (file path), not memory; uncovered surfaces flagged `[NEEDS HUMAN INPUT]` | Important |
| K3 | Drift threshold | Any tracked module | Module drift is within its status threshold (see `RULES.md`) | Important |
| K4 | Coverage-state accuracy | Coverage-state change proposed | The recorded manifest coverage state matches the evidence; promotion rules honored | Medium |
...
| X2 | Observability | Logs/metrics adequate for the change; nothing sensitive logged | Medium |
| X3 | Rollout safety | Semver/deprecation handling is correct for a published package change | Important |
