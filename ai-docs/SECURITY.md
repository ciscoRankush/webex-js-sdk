...
|---|---|---|---|
| Unmaintained dependencies | supply-chain risk from stale deps | tracked in the dependency audit process/catalog (retained under `docs/dependency/`) — see the audit subsection below | `[NEEDS HUMAN INPUT]` |
...
### Unmaintained dependency audit (supply-chain)

Supply-chain risk from stale/unmaintained npm dependencies is managed by a manual, repeatable, auditable process whose full workflow and per-package records are retained under `docs/dependency/` (not duplicated here). Discovery is source-aware across the whole monorepo: it collects direct `dependencies` and `devDependencies` from the root `package.json`, `packages/*/package.json`, and `packages/*/*/package.json`, attributing each dependency to its originating package.

- **Process & workflow:** [`docs/dependency/audit-process.md`](../docs/dependency/audit-process.md) — cadence (monthly detection of newly stale/suspect deps, quarterly catalog review; a designated developer runs the script and does manual triage), detection criteria (no npm publish for `STALE_YEARS` — default 2 years, marked `deprecated` on npm, or no visible upstream maintenance), severity scale (Critical = deprecated and no updates > STALE_YEARS; High = deprecated or no updates > STALE_YEARS; Medium = no updates > STALE_YEARS/2; Low = small low-impact utility), per-package decision (`REPLACE` preferred, `FORK_MAINTAIN`, `ACCEPT` with justification/monitoring, or `False Positive`), required triage evidence (npm last-publish timestamp, GitHub/repo activity, `deprecated` flag, CVE/OSV links), and the rule that `REPLACE`/`FORK_MAINTAIN` land via PR carrying version delta, rationale, evidence links, decision type, compatibility/tests/rollback, and a next-review date (~1 month). Target posture: Critical = 0, reduce High quarter over quarter.
- **Audit script:** [`docs/dependency/audit-unmaintained.js`](../docs/dependency/audit-unmaintained.js) (env `STALE_YEARS` default 2, `CONCURRENCY` default 20). It queries npm metadata, flags deprecated/stale packages, appends findings to the catalog, and writes immutable snapshots; it never modifies code or opens PRs, leaving the `Decision` column blank for manual triage.
- **Triaged catalog (decision system of record):** [`docs/dependency/unmaintained-catalog.md`](../docs/dependency/unmaintained-catalog.md) — one row per flagged package (date, package, version, source package location, reason, evidence, severity, decision, notes).
- **Immutable per-run snapshots:** e.g. [`docs/dependency/snapshots/unmaintained-2026-01-22T07-06-32-565Z.md`](../docs/dependency/snapshots/unmaintained-2026-01-22T07-06-32-565Z.md), same row schema as the catalog. If a run finds nothing new, the snapshot records "No unmaintained dependencies detected". All substantive changes (decisions, forks, replacements) are made via PR with evidence.
