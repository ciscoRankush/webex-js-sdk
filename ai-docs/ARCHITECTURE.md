...
## WS6 References
No WS6 spec, platform-architecture, shared-service, or enterprise-architecture document is linked from
this repository. The `@webex/webex-core` foundation and its plugins are documented by the manifest-routed
module spec and the standing docs in this tree; no authoritative WS6 source was identified in this
assess-only pass. Confirming whether a WS6 platform-architecture document applies is [NEEDS HUMAN INPUT].
