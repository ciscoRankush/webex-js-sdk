...
registry, and the request stack; it also provides client-side storage management (bounded and
unbounded stores over pluggable adapters) and an event-driven (evented) foundation for
configuration, lifecycle, and state-change events. Every `plugin-*` and `internal-plugin-*` package
layers domain behavior on top of this single, layered core, keeping a clear separation of concerns
between plugin registration, HTTP request handling, authentication, storage management, and events.
