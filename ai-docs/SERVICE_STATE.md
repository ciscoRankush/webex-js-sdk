...
<!-- N/A sections (client SDK, not a running service): Current Endpoints — the SDK exposes no HTTP endpoints of its own; Rate Limits & Quotas — enforced by upstream Webex services, not this client; Key Metrics & Performance Targets — no service SLOs (telemetry emitted via internal-plugin-metrics); Feature Flags (current) — no repo-owned live flag registry found in committed evidence. -->
