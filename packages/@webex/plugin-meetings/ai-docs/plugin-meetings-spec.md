...
- `syncMeetings` will destroy cached Locus meetings not returned by Locus unless they share a
  ...
  `packages/@webex/plugin-meetings/src/meetings/index.ts`.
- Unit tests are slow: always run only the tests you care about — to run a single test or a set of tests,
  temporarily add `.only` in the test code (for example, `it.only('should do something', () => { /* test code */ });`),
  and always remove `.only` once you finish running the tests. Evidence:
